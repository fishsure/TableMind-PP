...
def compute_tool_call(extra_info=None):
    ...
    # R_tool = e^{-rho*s}(beta * I_success - C * N_turns^2)
    # TableMind++: rho=0.05, beta=0.5, C=0.01
    RHO = 0.05
    BETA = 0.5
    C_BASE = 0.01
    alpha_tau = math.exp(-RHO * steps)
    i_success = 1.0 if successes > 0 else 0.0
    final_tool_call_reward = alpha_tau * (BETA * i_success - C_BASE * (turns ** 2))
    return final_tool_call_reward
